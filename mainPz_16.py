...
class Main(tk.Frame):
    ...
    def init_main(self):
        ...
        self.btn_open_dialog = tk.Button(toolbar, text='Добавить работника', command=self.open_dialog, bg='#00a8f3', bd=0,
    ...
    def update_record(self, user_id, name, second, sex, old, prof, score):
        self.db.cur.execute("""UPDATE susers SET user_id=?, name=?, second=?, sex=?, old=?,prof=?, score=? 
        WHERE user_id=?""",
                            (user_id, name, second, sex, old, prof, score, self.tree.set(self.tree.selection()[0], '#1')))
        self.db.con.commit()
        self.view_records()
    ...
    def search_records(self, score):
        score = (score,)
        self.db.cur.execute("""SELECT * FROM susers WHERE score>=?""", score)
        [self.tree.delete(i) for i in self.tree.get_children()]
        [self.tree.insert('', 'end', values=row) for row in self.db.cur.fetchall()]
...
class DB:
    ...
    def insert_data(self, user_id, name, second, sex, old, prof, score):
        self.cur.execute("""INSERT INTO susers(user_id, name, second, sex, old, prof, score) 
